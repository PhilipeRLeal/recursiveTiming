...
class RecursiveTimer():
    ...
    def verbose(self):
        if self.reference == {}:
            ...
            SpeedUp = (self.NewTime[NewTimeKey] / self.reference[RefereceKey])
            print('Reference Time: ', self.reference)
            print('New Time: ', self.NewTime)
            print('Speed gain (new/old) {0}/{1}: {2:.2f}'.format(NewTimeKey, RefereceKey, SpeedUp))
            print('New-Time is faster Reference-time: ', self.NewTime[NewTimeKey] < self.reference[RefereceKey])
    ...
    def decorator_factory(self, argument='CPU', function=callable, *args, **kwargs):
        ...
        def decorator(function):
            def wrapper():
                ...
                print("Average time: {0:.2f}ms Std {1:.2f}ms".format(average, std))
                self.verbose()
